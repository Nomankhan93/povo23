# Upgrade Phase 2.1 → 2.2

Use the existing `/home/noman/projects/poem-phase1.1` project. The folder name does not need to match the app version. This patch requires package version 2.1.0 and all five unchanged earlier migrations. Apply Phase 2.1 first if still on an earlier version.

Save the ZIP at `\\wsl.localhost\Ubuntu\home\noman\projects\poem-phase2.2-patch.zip`. In Ubuntu:

```bash
cd /home/noman/projects/poem-phase1.1
node -p "require('./package.json').version"
git status --short
patch_dir=$(mktemp -d /tmp/poem-phase22.XXXXXX)
unzip -n /home/noman/projects/poem-phase2.2-patch.zip -d "$patch_dir"
...
Existing local environment settings remain unchanged. If missing, run `npm run env:local` after Supabase starts. Keep Supabase project ID `poem-phase11` and ports 55321–55324. Do not run `supabase db reset`, create a second database project or re-bootstrap the admin. Only the new phase22 migration is applied to an upgraded database.

Run the manual checklist in PHASE-2.2.md. The local smoke test covers existing Auth/Storage workflows, not the complete new survey browser workflow.
...
```bash
git diff --stat
git add README.md FILES.txt docs package.json package-lock.json src scripts supabase/migrations/20260916000100_phase22_registry_assistance.sql
git diff --cached --stat
git commit -m "Build POEM Phase 2.2 registry review and assistance ledger"
git push
```
