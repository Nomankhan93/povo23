...
try{
  ...
  await call('set_project_recruitment_plan',[project,20,2,'open','Configure wallet test recruitment',recruitment.version]);
  ...
  await as('ngo');
  const opportunity=await call('create_recruitment_opportunity',[project,'Paid wallet test','Recruit one paid collector',taluka,dates.opp_start,dates.opp_end,dates.reply,'paid','Structured terms',1,'Survey','Urdu','all','Available',true]);
  await as('worker');
  ...
  const unit=await call('claim_work_payable',[assignment,dates.today,'Completed approved field work for withdrawal test']);
  await as('ngo');
