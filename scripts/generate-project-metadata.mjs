...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.18.0 PRODUCT BOUNDARIES =====',
    ...
    '- Worker payable accounting, verified organization funding, project reservation and payable-to-finance reconciliation exist; 2.18.0 adds a development-only e-wallet withdrawal sandbox without live provider custody or transfer.',
    ...
    '- Personal payout methods in 2.18.0 are limited to JazzCash and Easypaisa e-wallets; bank accounts / IBAN payout paths are intentionally excluded.',
    '- E-wallet binding stores normalized account data behind RPC-only tables, returns masked account numbers to the UI, and uses pending/verified/rejected/suspended/unlinked lifecycle states.',
    '- Transaction PINs are server-hashed with the Supabase pgcrypto API; plaintext PINs are never persisted or returned.',
    '- Withdrawal requests reserve exact approved-but-unpaid payable allocations so the same entitlement cannot fund concurrent withdrawals.',
    '- The current provider is a clearly labelled POEM-Admin-only mock sandbox: wallet verification and processing/success/failure/reversal outcomes are simulated and idempotent; end users cannot self-authorize provider outcomes and no live JazzCash or Easypaisa API endpoint or credential is embedded.',
    '- Successful mock settlement creates existing work_payable_events payment records so the 2.17.2 finance bridge moves Committed to Spent; mock reversal creates existing payment_reversal events and restores the finance state.',
    '',
    '===== DATABASE BASELINE =====',
    ...
    '2.18.1 / 2.18.2 — add live JazzCash and Easypaisa provider adapters only after official credentials and callback/signature documentation are available; preserve the 2.18.0 provider-neutral withdrawal contract and finance bridge.',
