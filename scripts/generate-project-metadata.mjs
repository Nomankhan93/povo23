...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '2.19.7 completes the Partner NGO application UX and organization-logo foundation. The completion hotfix adds Review document visibility, in-place document replacement and consistent applicant-facing Under review status without changing database authorization.',
    'The next screen-level UI phase is the Field Worker Marketplace UX; routing/database/authorization redesign remains out of scope unless a concrete workflow requires it.',
