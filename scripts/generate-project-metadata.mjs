...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '2.20.0 completes the dedicated Field Worker daily workspace. 2.21.0 completes the Organization daily workspace. 2.22.0 completes the first dedicated FieldLance Staff Operations Home with role-aware read-only queues over existing authoritative sources.',
    'The next development phase is 2.23 Tasks / SLA / Escalation Center. Keep task derivation tied to existing authoritative workflows instead of creating parallel approval or payment state.',
