...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.19.7 PRODUCT BOUNDARIES =====',
    ...
    '- 2.19.4 stabilized the frontend foundation. 2.19.5 completed the user-facing FieldLance rebrand. 2.19.6 added the visual-system/navigation foundation. 2.19.7 redesigns Partner NGO onboarding into a five-step application and adds the guarded organization-logo lifecycle.',
    ...
    '- Active NGO Admins can stage organization-owned project drafts; FieldLance approval materializes exactly one existing operational survey_projects row.',
    '- Project drafts can use FieldLance-owned templates or approved templates owned by the same NGO; cross-NGO private template reuse is rejected server-side.',
    '- FieldLance survey managers retain direct project creation and receive guarded template/project review queues with changes-request/reject/approve decisions.',
    ...
    '2.19.3 completes the currently scoped beneficiary assistance lifecycle from assessed need through delivered support, structured follow-up/outcome and controlled close/reopen history.',
    '2.19.7 completes the Partner NGO application UX and organization-logo foundation. The next screen-level UI phase is the Field Worker Marketplace UX; routing/database/authorization redesign remains out of scope unless a concrete workflow requires it.',
    'Audit the validated 2.19.3 state before choosing the next product phase; do not silently expand case closure into inventory, beneficiary cash-transfer execution, automated eligibility or finance.',
    ...
    '.git/, node_modules/, dist/, .poem-patch-backups/, .fieldlance-patch-backups/, supabase/.temp/, supabase/.branches/, local .env files, logs and temporary test directories are intentionally excluded.',
    '',
  ];
