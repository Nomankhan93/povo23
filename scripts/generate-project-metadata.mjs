...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.40.0 PRODUCT BOUNDARIES =====',
    ...
    '- Authoritative optional Polygon/MultiPolygon GeoJSON boundaries enable point-in-area review signals. When a boundary is missing, FieldLance returns unable_to_determine rather than guessing; geographic flags are review signals, never automatic fraud findings.',
    '- The personal /app/field/map surface is own-evidence only. Project managers see authorized project evidence, while Area Focal map access remains constrained by assigned project geography. Continuous/background worker tracking is not introduced.',
    '- Delegated case access requires an explicit active case assignment plus current project/geography authority. Delegation does not grant assistance approval, finance, Organization settings or broad beneficiary access.',
    ...
    'Future releases must use versions greater than 2.40.0. The previously planned 2.32-2.35 scopes remain deferred and should be renumbered when implemented.',
