...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.17.1 PRODUCT BOUNDARIES =====',
    ...
    '- Existing worker payable tables remain a separate entitlement subledger; 2.17.1 adds controlled organization funding and project reservation but no payable-to-finance bridge or provider settlement.',
    ...
    '- Verified funding sources/receipts are immutable. Available/reserved/committed/spent values are derived from standardized finance accounts rather than editable balance columns.',
    '',
    '===== DATABASE BASELINE =====',
    ...
    '2.17.2 — Payable → Finance Bridge & Reconciliation should post existing payable events into the finance ledger idempotently without recalculating worker entitlement.',
    'Keep 2.14 project staff / collection-geography authorization as the operational boundary for approved project workflows.',
    'Keep project funding/reservation on the 2.17.1 controlled RPC surface; do not widen generic finance posting to NGO or project staff.',
