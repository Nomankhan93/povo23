...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.19.4 PRODUCT BOUNDARIES =====',
    ...
    '- 2.19.4 is migration-free frontend foundation stabilization: grouped navigation coverage is corrected, duplicate Pager implementations are consolidated, and confirmed dead frontend files are removed without changing authorization or workflow semantics.',
    '- Case intake requires approved survey provenance; one assessed need can belong to only one active case at a time.',
    '- POEM survey authority, NGO Admin and Project Manager manage cases/requests/plans/follow-ups within existing project scope; only NGO Admin / POEM survey authority approve or reject requests. Area Focal remains outside broad case/planning/delivery/follow-up mutation authority.',
    ...
    '2.19.4 stabilizes the frontend foundation before screenshot-driven UI/UX work; large AppShell/CSS/routing refactors remain deferred until concrete UX issues are reviewed.',
