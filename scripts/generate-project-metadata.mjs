...
function generateAnalysisContext(files) {
  ...
  const lines = [
    ...
    '',
    '===== CURRENT 2.18.1 PRODUCT BOUNDARIES =====',
    ...
    '- Worker payable accounting, verified organization funding, project reservation and payable-to-finance reconciliation exist; 2.18.1 stabilizes the development-only e-wallet withdrawal sandbox without live provider custody or transfer.',
    ...
    '- Personal payout methods in 2.18.1 remain limited to JazzCash and Easypaisa e-wallets; bank accounts / IBAN payout paths are intentionally excluded.',
    ...
    '- The current provider is a clearly labelled POEM-Admin-only mock sandbox: verification/activation/provider events are event-keyed, end users cannot self-authorize provider outcomes, and no live JazzCash or Easypaisa API endpoint or credential is embedded.',
    '- Successful mock settlement is authorized by exact server-generated allocation request IDs, creates existing work_payable_events payment records so the 2.17.2 finance bridge moves Committed to Spent, and no longer trusts a custom settlement GUC; mock reversal uses the paired reversal request identity.',
    '',
    '===== DATABASE BASELINE =====',
