...
vm.runInNewContext(readFileSync('dist/field-sw.js','utf8'),{self,URL,Promise,caches:{open:async()=>cache,keys:async()=>['unrelated-app','poem-field-shell-old'],delete:async key=>deleted.push(key)},fetch:async()=>({network:true})});
...
handlers.activate({waitUntil:p=>work=p});await work;assert.deepEqual(deleted,['poem-field-shell-old']);
