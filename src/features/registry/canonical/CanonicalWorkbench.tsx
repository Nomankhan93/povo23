...
export function CanonicalWorkbench(){
 ...
 return <div className="canonical-workbench"><section className="panel detail"><h1>Canonical registry</h1><p>FieldLance operator workspace for master identities, project source records and reviewed matches. Access and review actions are audited.</p>
 <form className="canonical-search" onSubmit={search}><label className="field">Name, beneficiary number or canonical UUID<input name="query" maxLength={200} placeholder="FL-BEN-00000001 or name"/></label><label className="field">View<select name="state" defaultValue="active"><option value="active">Current identities</option><option value="review_required">Review required</option><option value="merged">Merged records</option><option value="all">All identities</option></select></label><button disabled={busy}>Search</button></form>
