...
type WalletRow={id:string;user_id:string;user_name:string;provider:'jazzcash'|'easypaisa';account_title:string;account_masked:string;status:string;created_at:string;version:number};
...
type Queue={wallets:WalletRow[];withdrawals:WithdrawalRow[];provider_mode:'mock'};
...
export function MockEWalletSandbox(){
 const [data,setData]=useState<Queue>({wallets:[],withdrawals:[],provider_mode:'mock'}),[busy,setBusy]=useState(false),[loading,setLoading]=useState(true),[error,setError]=useState(''),[notice,setNotice]=useState(''),[revision,setRevision]=useState(0);
 ...
 return <section className="ewallet-workspace">
  ...
  <div className="notice warning"><strong>No live money movement:</strong> these controls simulate JazzCash/Easypaisa ownership checks and callbacks for development testing. End users cannot invoke provider outcomes.</div>
  ...
  <section className="panel detail"><h3>Wallet verification queue</h3>{!data.wallets.length&&<EmptyState>No pending/rejected e-wallets.</EmptyState>}
   <div className="ewallet-grid">{data.wallets.map(w=><article className="document-row ewallet-card" key={w.id}><div className="panel-title"><div><strong>{w.user_name||w.user_id}</strong><p>{label(w.provider)} · {w.account_masked} · {w.account_title}</p></div><Badge value={w.status}/></div><div className="actions"><button className="secondary" disabled={busy} onClick={()=>void run(()=>call('simulate_mock_e_wallet_verification',{p_wallet:w.id,p_outcome:'verified',p_event_key:crypto.randomUUID()}),'Mock wallet verified.')}>Simulate verified</button><button className="secondary" disabled={busy} onClick={()=>void run(()=>call('simulate_mock_e_wallet_verification',{p_wallet:w.id,p_outcome:'rejected',p_event_key:crypto.randomUUID()}),'Mock wallet rejected.')}>Simulate rejected</button></div></article>)}</div>
  </section>
