...
export type LibraryTemplate={id:string;name:string;description:string;version:number;questions:Question[]};
export const templateLibrary: readonly LibraryTemplate[] = [
  {
    "id": "education",
    "name": "Education",
...
export function copyQuestions(qs:Question[]):Question[]{
...
export function dependencyErrors(qs:Question[]):string[]{
 return qs.flatMap((q,i)=>{
 const earlier=qs.slice(0,i),errors:string[]=[];
 if(q.when){const parent=earlier.find(p=>p.id===q.when!.question);if(!parent||(parent.type!=='yesno'&&parent.type!=='choice')||(parent.type==='yesno'?typeof q.when.equals!=='boolean':!parent.options?.includes(String(q.when.equals))))errors.push(`Question ${i+1}: fix its show-only-when condition.`);}
 if(q.after&&!earlier.some(p=>p.id===q.after&&p.type==='date'))errors.push(`Question ${i+1}: fix its date comparison.`);
 return errors;
 });
}
